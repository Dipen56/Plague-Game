...
public class AnotherRendering {
     ...
     * @param player
     */
    public AnotherRendering(Player player, Map<Integer, Player> playersOnMap,
    ...
    public void renderNorth() {
        ...
        for (int i = 0; i < squaresInFront; i++) {
            ...
            makeFrontSquares(i, tileWidth, topLine_Tile, nowStartX, nowWidthOfSquare,
                    x2_Tile, x3_Tile, y2_Tile, y2_Tile);
            for (int j = 0; j <= squaresToLeft; j++) {
                makeLeftSquares(j, i, y2_Tile, y2_Tile, x3_Tile, x2_Tile, nowStartX,
                        nowWidthOfSquare, topLine_Tile, tileHeight, scaleY);
            }
            for (int j = 0; j < squaresToRight; j++) {
                makeRightSquare(j, i, x2_Tile, x3_Tile, y2_Tile, y2_Tile,
                        nowWidthOfSquare, topLine_Tile, nowStartX, scaleY);
            }
    ...
    public void renderEast() {
        ...
        for (int i = 0; i <= squaresInFront; i++) {
            ...
            double nowStartX = centerWidth - nowWidthOfSquare / 2;
            makeFrontSquares(i, tileWidth, topLine_Tile, nowStartX, nowWidthOfSquare,
                    x2_Tile, x3_Tile, y2_Tile, y3_Tile);
            for (int j = 0; j <= squaresToLeft; j++) {
                makeLeftSquares(j, i, y3_Tile, y2_Tile, x3_Tile, x2_Tile, nowStartX,
                        nowWidthOfSquare, topLine_Tile, tileHeight, scaleY);
            }
            for (int j = 0; j <= squaresToRight; j++) {
                makeRightSquare(j, i, x2_Tile, x3_Tile, y3_Tile, y2_Tile,
                        nowWidthOfSquare, topLine_Tile, nowStartX, scaleY);
            }
            calculateTilePoints(nowStartX, i, nowWidthOfSquare);
        }
    ...
    public void renderObjectsNorth() {
        ...
        for (int frontSq = playerLoc.y; frontSq < 10; frontSq++) {
            double nowWidthOfSquare = tileWidth
                    * Math.pow(scaleY, (frontSq - playerLoc.y) + 1);
            double nowStartX = centerWidth - nowWidthOfSquare / 2;
            if (isInMap(frontSq, col)) {
                double x0 = nowStartX;
                double y0 = topLine_Object
                        - tileHeight * Math.pow(scaleY, (frontSq - playerLoc.y) + 1);
                getSurroundings(worldMap[frontSq][col], (x2_Object - x3_Object),
                        (y3_Object - y0), x0, y0);
            }
            for (int leftSq = 0; leftSq <= squaresToLeft; leftSq++) {
                if (isInMap((frontSq), (col + leftSq))) {
                    ...
                    double tempLefty0 = topLine_Object
                            - tileHeight * Math.pow(scaleY, (frontSq - playerLoc.y) + 1);
                    getSurroundings(worldMap[frontSq][col + leftSq],
                            tempLeftx2 - tempLeftx3, y3_Object - tempLefty0, tempLeftx0,
                            tempLefty0);
                }
            }
            for (int rightSq = 0; rightSq < squaresToRight; rightSq++) {
                if (isInMap((frontSq), (col - rightSq))) {
                    ...
                    double tempRighty0 = topLine_Object
                            - tileHeight * Math.pow(scaleY, (frontSq - playerLoc.y) + 1);
                    getSurroundings(worldMap[frontSq][col - rightSq],
                            (tempRightx2 - tempRightx3), (y3_Object - tempRighty0),
                            tempRightx0, tempRighty0);
                }
            }
            x3_Object = nowStartX;
            y3_Object = topLine_Object
                    - tileHeight * Math.pow(scaleY, (frontSq - playerLoc.y) + 1);
            x2_Object = nowStartX + nowWidthOfSquare;
            topLine_Object = topLine_Object
                    - tileHeight * Math.pow(scaleY, (frontSq - playerLoc.y) + 1);
    ...
    public void renderObjectsSouth() {
        ...
        for (int frontSq = 0; frontSq < squaresInFront; frontSq++) {
            ...
            if (isInMap(((playerLoc.y - 1) - frontSq), col)) {
                ...
                getSurroundings(worldMap[(playerLoc.y - 1) - frontSq][col],
                        (x2_Object - x3_Object), (y3_Object - y0), x0, y0);
            ...
            for (int leftSq = 0; leftSq < squaresToLeft; leftSq++) {
                if (isInMap(((playerLoc.y - 1) - frontSq), (col - leftSq))) {
                    ...
                    double lefty0 = topLine_Object
                            - tileHeight * Math.pow(scaleY, frontSq + 1);
                    getSurroundings(worldMap[(playerLoc.y - 1) - frontSq][col - leftSq],
                            (leftx2 - leftx3), (y3_Object - lefty0), leftx0, lefty0);
                }
            }
            for (int rightSq = 0; rightSq <= squaresToRight; rightSq++) {
                if (isInMap(((playerLoc.y - 1) - frontSq), (col + rightSq))) {
                    ...
                    double righty0 = topLine_Object
                            - tileHeight * Math.pow(scaleY, frontSq + 1);
                    getSurroundings(worldMap[(playerLoc.y - 1) - frontSq][col + rightSq],
                            (rightx2 - rightx3), (y3_Object - righty0), rightx0, righty0);
                }
            }
            x3_Object = nowStartX;
    ...
    /**
     * this method is used to render the object in the game that are to the north of the
     * player..
     */
    public void renderObjectsEast() {
        ...
        for (int frontSq = 0; frontSq <= squaresInFront; frontSq++) {
            ...
            if (isInMap((playerLoc.y), (playerLoc.x - frontSq))) {
                double x0 = nowStartX;
                double y0 = topLine_Object - tileHeight * Math.pow(scaleY, frontSq + 1);
                getSurroundings(worldMap[playerLoc.y][playerLoc.x - frontSq],
                        (x2_Object - x3_Object), (y3_Object - y0), x0, y0);
            }
            for (int leftSq = 0; leftSq <= squaresToLeft; leftSq++) {
                if (isInMap((playerLoc.y + leftSq), (col - frontSq))) {
                    ...
                    double lefty0 = topLine_Object
                            - tileHeight * Math.pow(scaleY, frontSq + 1);
                    getSurroundings(worldMap[playerLoc.y + leftSq][col - frontSq],
                            (leftx2 - leftx3), (lefty3 - lefty0), leftx0, lefty0);
                }
            }
            for (int rightSq = 0; rightSq <= squaresToRight; rightSq++) {
                if (isInMap((playerLoc.y - rightSq), (col - frontSq))) {
                    ...
                    double righty0 = topLine_Object
                            - tileHeight * Math.pow(scaleY, frontSq + 1);
                    getSurroundings(worldMap[playerLoc.y - rightSq][col - frontSq],
                            (rightx2 - rightx3), (righty3 - righty0), rightx0, righty0);
                }
            }
            x3_Object = nowStartX;
            y3_Object = topLine_Object - tileHeight * Math.pow(scaleY, frontSq + 1);
    ...
    public void getSurroundings(String objectName, double width, double height, double x,
            double y) {
        ImageView imageView = new ImageView();

        switch (objectName) {
        case "tree":
            imageView.setImage(Images.TREE_IMAGE);
            break;
        case "chest":
            imageView.setImage(Images.CHEST_IMAGE);
            break;
        }
    ...
    /**
     * Checks if the object in front or beside the player is a valid object in the map
     * 
     * @param row
     * @param col
     ...
     * @param scaleY
     */
    public void makeLeftSquares(int row, int col, double y3, double y2, double x3,
            double x2, double nowStartX, double nowWidthOfSquare, double topLine,
            double tileHeight, double scaleY) {
    ...
    public void makeRightSquare(int j, int i, double x2, double x3, double y3, double y2,
            double nowWidthOfSquare, double topLine, double nowStartX, double scaleY) {
        double previouWidthOfSquare = (x2 - x3);
    ...
    public void makeFrontSquares(int i, double tileWidth, double topLine,
            double nowStartX, double nowWidthOfSquare, double x2, double x3, double y2,
            double y3) {
     ...
     * @param y4
     */
    public void makePolygonTile(double x1, double y1, double x2, double y2, double x3,
            double y3, double x4, double y4) {

        Polygon polygon = new Polygon();
        polygon.setFill(new ImagePattern(Images.GRASS_IMAGE));
    ...
    public void setBackground() {
        ...
        imageViewNight.setImage(Images.BACKGROUND_IMAGE);
    ...
    /**
     * Initializes the points that are used when drawing the objects in the graphics pane
     */
     ...
     */
    public void calculateTilePoints(double nowStartX, int Nsquare,
            double nowWidthOfSquare) {
        x3_Tile = nowStartX;
     ...
     * the render method. so when a player makes a move there stats will be updated and
     * based on this a string will be passed containg this update information which is
     * used to render the player in tand the objectss in the right place.
     * 
     * @param player
     */
