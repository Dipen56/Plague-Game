...
public class AntherRendering_2 {
     ...
     * @param player
     */
    public AntherRendering_2(Player player, Map<Integer, Player> playersOnMap,
    ...
    public void renderNorth() {
        ...
        Image image = Images.BACKGROUND_IMAGE;
    ...
    public void renderObjectsNorth() {
        ...
        for (int row = playerLoc.y; row < 10; row++) {
            ...
            double nowWidthOfSquare = tileWidth
                    * Math.pow(scaleY, (row - playerLoc.y) + 1);
            // this is the top part of the tile starting x pos
            double nowStartX = centerWidth - nowWidthOfSquare / 2;
            // front view
            if (worldMap[row][col].equals("tree") || worldMap[row][col].equals("chest")) {
                ...
                double y1 = topLine
                        - tileHeight * Math.pow(scaleY, (row - playerLoc.y) + 1);
                double x0 = nowStartX;
                double y0 = topLine
                        - tileHeight * Math.pow(scaleY, (row - playerLoc.y) + 1);
                double height = (y3 - y0);
            ...
            for (int j = 0; j < squaresToRight; j++) {
                if (worldMap[row][col - j].equals("tree")
                    ...
                    double tempRightY1 = topLine
                            - tileHeight * Math.pow(scaleY, (row - playerLoc.y) + 1);
                    double tempRightx0 = nowStartX + j * nowWidthOfSquare;

                    double tempRighty0 = topLine
                            - tileHeight * Math.pow(scaleY, (row - playerLoc.y) + 1);
            ...
            for (int j = 0; j < squaresToLeft; j++) {
                if (worldMap[(playerLoc.y - 1) - row][col - j].equals("tree")
                    ...
                    if (worldMap[(playerLoc.y - 1) - row][col - j].equals("tree")) {
                        ...
                        imageViewTree.setY(tempLefty0);
                        group.getChildren().add(imageViewTree);
                    } else if (worldMap[(playerLoc.y - 1) - row][col - j]
                            .equals("chest")) {
                        Image tree = Images.CHEST_IMAGE;
                        ImageView imageViewTree = new ImageView();
                        imageViewTree.setImage(tree);
     ...
     * used to render the player in tand the map<Interger user id, Avatar enum>
     * map<Integer user id, Position> objectss in the right place. //int x int y ,
     * Direction .. , int area ID
     * 
     * @param player
     */
