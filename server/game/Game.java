...
public class Game {
	...
	/**
	 * The visibility in daytime. This number indicates that everything within
	 * this distance on world grid is visible.
	 */
	public static final int DAY_VISIBLIITY = 7;

	/**
	 * The visibility in night time. This number indicates that everything
	 * within this distance on world grid is visible.
	 */
	public static final int NIGHT_VISIBILITY = 3;

	/**
	 * The visibility if the player is holding a torch in night time. This
	 * number indicates that everything within this distance on world grid is
	 * visible.
	 */
	 ...
	 */
	private static final int SUNRISE_TIME = 6;
	 ...
	 * <i>areas</i> field as well. I keep it here just for not bringing major
	 * changes to data storage component.
	 */
	private Area world;

	/**
	 * A table of all areas. Each area has its unique area id number. All areas
	 * and their corresponding id number is recorded in this table.
	 */
	...
	/**
	 * Constructor a game world without any players inside. Note that the first
	 * parameter is not really necessary, as the second parameter will contain
	 * the world map. It is a legacy parameter.
	 *
	 * @param world
	 *            --- The main game world.
	 * @param areas
	...
	public void joinPlayer(Player player) {
		...
		/*
		 * If player has a position, then it has been loaded from a previous
		 * game, and does not need a new position.
		 */
		...
		/*
		 * This should never happen. In theory, if the whole world doesn't have
		 * even one empty position left, this could happen. But that is almost
		 * impossible.
		 */
		if (pos == null) {
	...
	/**
	 * Start the world time. The world time is constantly advancing. As long as
	 * the server is running, no other events will stop it.
	 */
	...
	/**
	 * Disconnect the player, and re-distribute all his keys to locked
	 * containers.
	 *
	 * @param playerId
	 ...
	 * @return --- true if there is another player in that position; or false if
	 *         not.
	 */
	public boolean isOccupiedByOtherPlayer(Position position) {
		...
		for (Player p : players.values()) {
			...
			if (pos.areaId == position.areaId && pos.x == position.x && pos.y == position.y) {
				return true;
			}
	 ...
	 * @return --- true if successful, or false if the player cannot move
	 *         forward for some reason, e.g. blocked by obstacle.
	 */
	 ...
	 *            --- the id number of the player
	 * @return --- true if successful, or false if the player cannot move
	 *         backward for some reason, e.g. blocked by obstacle.
	 */
	 ...
	 * @param uid
	 *            --- the id number of the player
	 * @return --- true if successful, or false if the player cannot move left
	 *         for some reason, e.g. blocked by obstacle.
	 */
	 ...
	 * @param uid
	 *            --- the id number of the player
	 * @return --- true if successful, or false if the player cannot move right
	 *         for some reason, e.g. blocked by obstacle.
	 */
	...
	public void playerTurnLeft(int uid) {
		...
		// When transit we should keep player's direction unchanged.
		Position currentPosition = player.getPosition();
		Area currentArea = areas.get(currentPosition.areaId);
		MapElement currentMapElement = currentArea.getMapElementAt(currentPosition.x, currentPosition.y);
		if (currentMapElement instanceof TransitionSpace) {
			player.setPosition(new Position(currentPosition.x, currentPosition.y, currentPosition.areaId,
					player.getPosition().getDirection().left()));
			return;
		}
	...
	public void playerTurnRight(int uid) {
		...
		if (currentMapElement instanceof TransitionSpace) {
			player.setPosition(new Position(currentPosition.x, currentPosition.y, currentPosition.areaId,
					player.getPosition().getDirection().right()));
			return;
		}
	...
	/**
	 * This method let the given player try to transit between areas (enter or
	 * exit a room).
	 *
	 * @param uid
	 *            --- the id number of the player
	 * @return --- true if the player changed to another area, or false if this
	 *         action failed for some reason, for example the player is not
	 *         facing the door, or he is too far from it.
	 */
	...
	/**
	 * This method let the given player try to unlock a chest, room, or other
	 * lockable object in front.
	 *
	 * @param uid
	 *            --- the id number of the player
	 * @return --- true if the loackable is unlocked, or false if this action
	 *         failed. Failure can be caused by many reasons, for example it's
	 *         not a lockable in front, or the player doesn't have a right key
	 *         to open it.
	 */
	public boolean playerUnlockLockable(int uid) {
		...
		if (currentMapElement instanceof TransitionSpace) {
			...
			if (player.getDirection() == currentTransition.getFacingDirection() && destArea instanceof Room) {
	...
	/**
	 * This method let the player try to take items from the container in front.
	 * If the player's inventory can take in all items, he will take them all;
	 * otherwise he will take as many as he can until his inventory is full.
	 *
	 * @param uid
	 *            --- the id number of the player
	 ...
	 *         {@link server.game.Game #DAY_VISIBLIITY DAY_VISIBLIITY} at day
	 *         time, {@link server.game.Game #NIGHT_VISIBILITY NIGHT_VISIBILITY}
	 *         at night time if the player isn't holding a burning torch, and
	 *         {@link server.game.Game #TORCH_VISIBILITY TORCH_VISIBILITY} at
	 *         night time if the player is holding a burning torch.
	 */
	public int getPlayerVisibility(int uid) {
		Player player = players.get(uid);

		if (clock.getHour() >= SUNRISE_TIME && clock.getHour() < SUNSET_TIME) {
			// it's day time
	 ...
	 *
	 * @return --- the table of areas as a Map where the key is areaId, and the
	 *         value is area.
	 */
	...
	/**
	 * This method returns the clock of the world. The world time is constantly
	 * advancing.
	 *
	...
	/**
	 * This method is used to generate the string for broadcasting world time to
	 ...
	 *
	 * @return --- a string representation of the world time. This is used for
	...
	/**
	 * Generate a String of all players' avatars so the client who get this
	 * string will know other player's avatar. The String has the following
	 * format: <i>"uId_1,avatar_index_1|uId_2,avatar_index_2"</i>
	 *
	 * <p>
	 ...
	 *
	 * @return --- a string representation of all players and their chosen
	 *         avatars. This is used for network transmission.
	 */
	...
	/**
	 * Generate a String of the status of all players' torch, i.e. whether he is
	 * holding a (lighted) torch or not. This is used for the renderer at client
	 * side. The String has the following format:
	 ...
	 *
	 * @return --- a string representation of the status of all players' torch.
	 *         This is used for network transmission.
	 */
