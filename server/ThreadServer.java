...
public class ThreadServer extends Thread {
    ...
    public void run() {
        ...
        while (true) {
            ...
            try {
                ...
                consoleMessage = "Client connected - socket address : "
                        + recievedData.getSocketAddress();
    ...
    /**
     * Extracts the type of packet the server had received and calls the appropriate
     * method to handle the data
     * 
     * @param recievedPacket
     *            to be parsed and extracted
     */
    public void readPacket(DatagramPacket recievedPacket) {
        ...
        if (type.equals(DataType.LOGIN))
            ...
            // sendData(packetType.new Message(("I have recieved your message client: " +
            // clientIP ).getBytes()).getMessage() , clientIP, portNum);
    ...
    /**
     * Broadcasts to everyone in the network that a new user has joined the game by making
     * a new Message packet and attaching the name of the player
     * 
     * @param data
    ...
    public void handleLogIn(byte[] data, InetAddress ipAddress, int portNum) {
        ...
        sendData(packetType.new Message("You are now connected to the server".getBytes())
                .getMessage(), ipAddress, portNum);
    ...
    private void addClient(String username, InetAddress hostAddress, int port) {
        ...
        ClientInformation playerData = new ClientInformation(port, username, hostAddress,
                clientNum);
