...
public class InitialGameLoader {
	...
	private static GroundSpace g = new GroundSpace(); // For a plain piece of
														// ground.
	// Obstacles which are used more than once.
	/**
	 * An empty "room square".
	 */
	private static Obstacle e = new Obstacle("Looks like a room");

	/**
	 * A tree.
	 */
	private static Obstacle t = new Obstacle("It's a tree");

	/**
	 * A rock.
	 */
	private static Obstacle r = new Obstacle("Just a big, old rock.");

	/**
	 * A chair.
	 */
	private static Obstacle c = new Obstacle("A chair. That won't cure me.");

	/**
	 * A barrel.
	 */
	private static Obstacle b = new Obstacle("A big round barrel. It's empty.");

	/**
	 * A table.
	 */
	private static Obstacle tb = new Obstacle("A table.");
	 ...
	 * Objects 12-13 are in room3.
	 * Objects 14-17 are in room4
	 *
	 * Cupboards:
	 * Cupboard 0 is in room1.
	 * Cupboards 1-2 are in room2.
	 * Cupboards 3-4 are in room3.
	 * Cupboard 5 is in room4.
	 *
	 * @return The game.
	 */
	public static Game makeGame() {
		...
		initialiseListLootLists(scrapLoot, NUMBER_OF_S_PILES); // Initialises chest loot lists.
		...
		for (int i = 0; i < NUMBER_OF_CUPBOARDS; i++) {
			cp[i] = new Cupboard("A cupboard. It might contain some medicine.",
					keys.get(NUMBER_OF_CHESTS + i).getKeyID(), false, cupboardLoot.get(i));
	...
	public static void allocateRoomKeys() {
		...
		for (int i = 0; i < NUMBER_OF_ROOMS; i++) {
			...
			if (Math.random() * 10 > 5) {
				...
				// Adds key to random scrap pile. Generates index of scrap pile. Value 9 is number of  scrap piles  outside.
	...
	private static void addContainerKeysToContainers(){
		...
		for(int i = 0 ; i < NUMBER_OF_CHESTS + NUMBER_OF_CUPBOARDS; i++){
			...
			if(rand > 5){
				//Chest
				do{
					containerIndex = (int)(Math.random() * NUMBER_OF_CHESTS);	//The index of the chest that gets the key.
