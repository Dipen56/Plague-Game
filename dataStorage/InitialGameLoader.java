...
public class InitialGameLoader {
	...
	private static GroundSpace g = new GroundSpace();	// For a plain piece of ground.
	//Obstacles which are used more than once.
	private static Obstacle e = new Obstacle("Looks like a room");	//An empty "room square"
	private static Obstacle t = new Obstacle("It's a tree");	//A tree
	private static Obstacle r = new Obstacle("Just a big, old rock.");	//A rock
	private static Obstacle c = new Obstacle("A chair. That won't cure me.");	//A chair
	private static Obstacle b = new Obstacle("A big round barrel. It's empty.");	//A barrel
	private static Obstacle tb = new Obstacle("A table.");	//A table
	 ...
	 * 
	 * Keys to be distributed are ordered in the order: chest keys, cupboard keys, room keys.
	 * So in order to get the key id for a certain cupboard you will need the index of the cupboard relative to the the other cupboards, and also the number of chest keys.
	 * 
	 * KeyIDs are allocated based on their index in the keys list.
	 * KeysIDs 0-15 are for outdoor chests
	 * KeysIDs 16-22 are for indoor chests.
	 * KeysIDs 23-28 are for cupboards.
	 * 
	 * The following locations are required for mapping objects to the game maps.
	 * Chest locations:
	 * Chests 0-15 are outdoors.
	 * Chest 16 is in room 2
	 * Chests 17-21 are in room 3
	 * Chest 22 is in room 4
	 * 
	 * ScrapPiles:
	 * Objects 0-9 are outside
	 * Objects 10-11 are in room1
	 * Objects 12-15 are in room4
	 * Objects 16-17 are in room4
	 * 
	 * @return The game.
	 */
	public static Game makeGame(){
		...
		initialiseListLootLists(chestLoot, NUMBER_OF_CHESTS);	//Initialises chest loot lists.
		initialiseListLootLists(cupboardLoot, NUMBER_OF_CUPBOARDS);	//Initialises cupboard loot lists.
		initialiseListLootLists(scrapLoot, NUMBER_OF_S_PILES);	//Initialises ches loot lists.


		//Area ID numbers are generated
		areaIDs.add(-1);	//This allows TransitionSpace numbers to match indices. Also index 0 is the main map.
		Integer id = 0;
		for(int i = 0; i < NUMBER_OF_ROOMS; i++){
			do{
				id = (int)(Math.random()*29);
		...
		Key key = null;
		int keyID = -1;
		//Creates a key based on a random number. If the list contains that same key, make a new one.
		for(int i = 0; i < NUMBER_OF_CHESTS + NUMBER_OF_CUPBOARDS + NUMBER_OF_ROOMS; i++){
			do{
				keyID = (int)(Math.random()*29);
		...
		for(int i = 0; i < NUMBER_OF_CHESTS; i++){
			ch[i] = new Chest("A chest. Probably contains loot.", keys.get(i).getKeyID(), true, chestLoot.get(i));
			spawnAntidotes(ch[i].getLoot(), ANTIDOTE_CHEST_MULT);//Random chance of adding antidotes to Chest loot list
		}

		//Creates Cupboard and adds antidotes to them.
		Cupboard cupboard = null;
		for(int i = 0; i < NUMBER_OF_CUPBOARDS; i++){
			cp[i] = new Cupboard("A cupboard. It might contain some medicine.",keys.get(NUMBER_OF_CHESTS + i).getKeyID(),false, cupboardLoot.get(i)); 
		...
		MapElement[][] worldMap = {
				...
				{ 	g, 	g,ts1,	g, 	g, 	g, 	g, 	g, 	g, 	t, 	r, 	r, 	r, 	g, 	g, s[2],  g, 	g, 	r, 	r, 	r, 	r, 	g, 	g, 	g, 	g, 	r, 	r, 	g, 	g},
				...
				{ 	t, 	g, 	g, 	g, 	g,	g, 	r, 	r, 	r,	r, 	g, 	g, 	t, 	e, 	e, 	e, 	e, 	t, 	g, 	g,	g, 	t, 	g, 	g, 	g, 	e, 	e, 	e, 	e, 	t},
				{ 	g, 	g, 	g, 	g, 	g, 	g, 	r,ch[9], 	g, 	g, 	g, 	r, 	t, 	e, 	e, 	e, 	e, 	t, 	g, 	g, 	g, 	t, 	g, 	g, 	g, 	e, 	e, 	e, 	e, 	t},
				{ 	t, 	t, 	t, 	g, 	r, 	r, 	g, 	g, 	g, 	g, 	g, 	r, 	g,ts2, 	g, 	g, 	g, 	g, 	g, 	g, 	g, 	t, 	g, 	g, 	g, 	e, 	e, 	e, 	e, 	t},
				...
				{ 	t, 	g, 	r, 	g, 	t, 	g, 	g, 	g,ts4, 	t, 	g, 	g, 	g, 	g, 	g, 	g, 	g, 	g, 	g, 	g,s[9], 	t, 	g, 	r, 	g, 	g,ch[12], 	r, 	g, 	g},
	 ...
	 * @param The list of lists of items.
	 * @param The number of lists to initialise inside the list of lists.
	 */
	 ...
	 * @param An array of Chests.
	 * @param An array of Cupboards.
	 * @param A list of keys.
	 */
	public static void allocateRoomKeys(){
		int containerIndex = 0, keyIndex = 0;
		for(int i = 0; i < NUMBER_OF_ROOMS; i++){
			keyIndex = keys.size()-i;
			if(Math.random() * 10 > 5){
				//Adds key to random outdoor chest
				containerIndex = (int)(Math.random()*15+1); 	//generates index of chest.
				ch[containerIndex].getLoot().add(keys.get(keyIndex));
			}
			else{
				//Adds key to random scrap pile
				containerIndex = (int)(Math.random()*NUMBER_OF_S_PILES+1); 	//generates index of scrap pile.
				cp[containerIndex].getLoot().add(keys.get(keyIndex));
			}
		}
	}

	/**
	 * Adds Antidote items to the loot list.
	 * The chance of adding 1 antidote = multiplier.
	 * The chance of adding 2 antidote = multiplier/2.
	 * The chance of adding 3 antidote = multiplier/3.
	 * @param The loot list
	 * @param A double used as a multiplier
	 */
	private static void spawnAntidotes(List<Item> loot, double multiplier){
		int result = 0;
		for(int i = 0; i < 3; i++){
			result = (int)(Math.random() * 10);
			//As the multiplier decreases, the less chance of adding an antidote.
	...
	private static void addContainerKeysToContainers(){
		...
		List<Integer> addedInAdvance = new ArrayList<>(); // Records which chests have had their key given out before i == index.
		for(int i = 0 ; i < NUMBER_OF_CHESTS + NUMBER_OF_CUPBOARDS; i++){
			//Randomly decides which sort of container will hold the key, out of Chest or ScrapPile
			int rand = (int)(Math.random() * 10);
			if(rand > 5){
				//Chest
				do{
					containerIndex = (int)(Math.random() * NUMBER_OF_CHESTS + 1);	//The index of the chest that gets the key.
				}while(containerIndex == i || addedInAdvance.contains(containerIndex));	// Prevents chest from holding its own key.
				ch[containerIndex].getLoot().add(keys.get(i));	//Adds the key to the chest.
				//Makes sure key to this chest is put inside a ScrapPile, if this chest's key has not yet been assigned.
				//The intention is to make chest access less reliant on keys contained in chests.
				if(i < containerIndex){
				...
				//ScrapPile
				[(int)(Math.random()*NUMBER_OF_S_PILES)].getLoot().add(keys.get(i));
				addedInAdvance.add(containerIndex);
			}
		}
	}
